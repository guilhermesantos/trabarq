...
for i in {1..10}
	do
		sudo perf stat -d ./bench_int_mat &>> result_int_mat_$PLAT_NAME.txt
	done

echo 'Fazendo testes de multiplicacao de matrizes float'
for i in {1..10} 
	do
		sudo perf stat -d ./bench_float_mat &>> result_float_mat_$PLAT_NAME.txt
	done

echo 'Fazendo testes de gauss'
for i in {1..10} 
	do
		sudo perf stat -d ./bench_float_gauss &>> result_float_gauss_$PLAT_NAME.txt
	done

echo 'Fazendo testes de branch sem sort'
for i in {1..10} 
	do
		sudo perf stat -d ./bench_branch &>> result_branch_$PLAT_NAME.txt
	done
